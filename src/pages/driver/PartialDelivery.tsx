...
export default function PartialDelivery() {
  ...
  const handlePartialDeliver = (orderId: string) => {
    if (!reason) {
      alert('الرجاء إدخال سبب التسليم الجزئي');
      return;
    }
    // In a real app, this would make an API call to mark as delivered 
    // and create a new returned order with `${orderId}P`
    setOrders(orders.map(o => o.id === orderId ? { ...o, status: 'delivered', totalAmount: Number(receivedAmount) } : o));
    setSelectedOrder(null);
    setReceivedAmount('');
    setReason('');
    alert(`تم تأشير الطلب كمسلم.\nتم استلام مبلغ (${receivedAmount} د.ع) متضمناً أجرة التوصيل.\nتم إنشاء طلب جديد للمواد الراجعة برقم ${orderId}P وإرساله إلى المخزن الرئيسي.`);
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            ابحث عن الطلب لتسليمه جزئياً وإدخال المبلغ المستلم والسبب
          </p>
        </div>
      </div>
                ...
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map((order) => (
                    <React.Fragment key={order.id}>
                      <tr className={`hover:bg-slate-50/80 transition-colors ${selectedOrder === order.id ? 'bg-orange-50/30' : ''}`}>
                        ...
                        <td className="px-6 py-4 text-center">
                          {order.status === 'delivering' && (
                             <button
                               ...
                               className="bg-orange-600 hover:bg-orange-700 text-white px-3 py-1.5 rounded-lg font-bold transition-colors whitespace-nowrap text-xs"
                             >
                               تسليم جزئي
                             </button>
                          )}
                      ...
                      {selectedOrder === order.id && order.status === 'delivering' && (
                        <tr>
                          <td colSpan={10} className="p-0 border-b border-orange-100 bg-orange-50/20">
                            <div className="p-6">
                              <div className="bg-white border border-orange-200 rounded-2xl p-6 shadow-sm max-w-3xl mx-auto space-y-6">
                                <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                                  ...
                                  <div>
                                    <h4 className="font-bold text-slate-800 text-base">تسليم جزئي للطلب {order.id}</h4>
                                  ...
                                  <div>
                                    ...
                                    <div className="relative">
                                      <textarea
                                        ...
                                        placeholder="اكتب هنا سبب إرجاع بعض المواد..."
                                        rows={3}
                                        className="w-full bg-white border-2 border-orange-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all font-medium text-slate-700"
                                      />
                                ...
                                <div className="flex justify-end gap-3 pt-4">
                                  ...
                                  <button
                                    onClick={() => handlePartialDeliver(order.id)}
