...
export default function ReturnOrder() {
  ...
  const handleReturn = (orderId: string) => {
    if (!reason.trim()) {
      alert('الرجاء إدخال سبب الإرجاع');
      return;
    }
    // In a real app, this would make an API call to mark as returned
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">إرجاع طلب</h1>
          <p className="text-slate-500 font-medium mt-1">
            ابحث عن الطلب لإرجاعه مع ذكر السبب
          </p>
        </div>
      </div>
                ...
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map((order) => (
                    <React.Fragment key={order.id}>
                      <tr className={`hover:bg-slate-50/80 transition-colors ${selectedOrder === order.id ? 'bg-red-50/30' : ''}`}>
                        ...
                        <td className="px-6 py-4 text-center">
                          {order.status === 'delivering' && (
                             <button
                               ...
                               className="bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg font-bold transition-colors whitespace-nowrap text-xs"
                             >
                               إرجاع الطلب
                             </button>
                          )}
                      ...
                      {selectedOrder === order.id && order.status === 'delivering' && (
                        <tr>
                          <td colSpan={10} className="p-0 border-b border-red-100 bg-red-50/20">
                            <div className="p-6">
                              <div className="bg-white border border-red-200 rounded-2xl p-6 shadow-sm max-w-3xl mx-auto space-y-6">
                                <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                                  <div className="w-10 h-10 bg-red-100 text-red-600 rounded-xl flex items-center justify-center shrink-0">
                                    <XCircle className="w-5 h-5" />
                                  </div>
                                  <div>
                                    <h4 className="font-bold text-slate-800 text-base">إرجاع الطلب {order.id}</h4>
                                    <p className="text-slate-500 text-sm">أدخل سبب الإرجاع لتأكيد العملية</p>
                                  </div>
                                </div>
                                
                                <div className="space-y-4">
                                  <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">سبب الإرجاع <span className="text-red-500">*</span></label>
                                    <div className="relative">
                                      <textarea
                                        value={reason}
                                        onChange={(e) => setReason(e.target.value)}
                                        placeholder="اكتب هنا سبب إرجاع الطلب (مثال: العميل رفض الاستلام)..."
                                        rows={3}
                                        className="w-full bg-white border-2 border-red-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-4 focus:ring-red-500/10 focus:border-red-500 transition-all font-medium text-slate-700"
                                      />
                                ...
                                <div className="flex justify-end gap-3 pt-4">
                                  ...
                                  <button
                                    ...
                                    className="px-6 py-2.5 rounded-xl font-bold bg-red-600 text-white hover:bg-red-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                                  >
