...
export default function WarehouseDriverCommission() {
  ...
  const driverAccounts = driversList.map((driver) => {
    // Only fetch orders that are delivered/partial delivered and haven't had commission paid yet.
  ...
  const handleConfirmSettlement = () => {
    ...
    // 1. Add payment transaction for commission
    addTransaction({
      type: 'payment',
    ...
    // 2. Mark orders as commission paid
    ...
    <div className="max-w-7xl mx-auto space-y-8" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 print:hidden">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-[#0F3B73]">تسليم عمولة المناديب</h1>
          <p className="text-slate-500 font-bold">إدارة وتسوية العمولات المستحقة للمناديب وعرض تفاصيلها واستخراج سندات الصرف</p>
        </div>
          ...
          <table className="w-full text-right whitespace-nowrap min-w-[700px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-sm">
                ...
                <th className="px-4 py-4 font-black text-slate-400">العمولة المستحقة للصرف</th>
            ...
            <tbody className="divide-y divide-slate-50 text-sm p-4">
              {filteredDrivers.length === 0 ? (
                <tr>
                   <td colSpan={3} className="px-4 py-12 text-center text-slate-400 font-bold">لا يوجد مناديب مستحقين لعمولات</td>
                </tr>
              ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 print:p-0 print:bg-white print:block">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden print:shadow-none print:w-full print:max-w-none">
            ...
            <div className="flex items-center justify-between p-4 border-b border-slate-100 print:hidden bg-slate-50">
              <h2 className="text-xl font-black text-[#0F3B73]">استخراج مستند صرف عمولة</h2>
              <button 
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="text-center space-y-2 border-b-2 border-dashed border-slate-300 pb-6">
                ...
                <div className="pt-2 text-sm font-bold text-slate-400">
                  رقم المستند: <span className="font-en">PAY-{Date.now().toString().slice(-6)}</span>
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                ...
                <div className="flex justify-between items-center mb-2">
                  <span className="text-slate-500 font-bold">عدد الطلبات المشمولة (واصل وواصل جزئي)</span>
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center p-4 rounded-2xl bg-slate-900 text-white">
                  <span className="text-lg font-black">الصافي لصرفه من الصندوق</span>
                  <span className="text-3xl font-black font-en tracking-tight">{selectedDriver.commission.toLocaleString()} د.ع</span>
                </div>
              </div>

              {/* Signatures */}
              <div className="flex justify-between mt-12 pt-8 text-center text-slate-600 font-bold">
                <div className="w-32 border-t-2 border-slate-300 pt-2">توقيع المستلم (المندوب)</div>
                <div className="w-32 border-t-2 border-slate-300 pt-2">توقيع مسؤول المخزن / الخزينة</div>
              </div>
            </div>
